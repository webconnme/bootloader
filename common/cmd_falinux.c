...
static char ezb_def_str[F_IDX_COUNT+1][80] = 
{
	"00:fa:15:21:20:07"       ,
	"192.168.10.236"          ,
	"255.255.255.0"           ,
...
static void setenv_common( void )
{
	...
	sprintf( str_env, "setenv bootargs ${%s} ${app_KCMD2} ${%s} ${%s} ${%s}", 
		ezb_key[F_IDX_KCMD1], ezb_key[F_IDX_KCMD3], ezb_key[F_IDX_KCMD4], ezb_key[F_IDX_KCMD5] );
...
static void rootfs_change( char mode )
{
	...
	switch ( mode )
	{
	case 'f' : case 'F' : sprintf( rfs_str, "%s", ezb_def_str[EZB_DEF_STR_RFS_NAND_IDX]    ); break;
	case 'r' : case 'R' : sprintf( rfs_str, "%s", ezb_def_str[EZB_DEF_STR_RFS_RAMDISK_IDX] ); break;
	default  : return;
	}
...
static int do_set(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	while( 1 )
	{
		...
		if ( 0 < nbytes ) 
		{
			...
			case 'l' : case 'L' : rootfs_change( ptr[1] ); break;
			}
...
static void write_to_nand( int addr, int size )
{
	...
	sprintf( str_cmd, "nand erase 0x%x 0x%x",
		addr,
		size
	);
	printf( "  >>%s\n",	str_cmd );
...
static void saveenv_after_download( void )
{
	...
	{
		// env, bootm
		sprintf( str_env, "%s; %s; bootm 0x%x",
			getenv( KEY_LOAD_K ),
			getenv( KEY_LOAD_R ),
			DRAM_KERNEL_BOOTM
		);		
	...
	{
		// env, bootm
		sprintf( str_env, "%s; %s; %s; bootm 0x%x - 0x%x",
			getenv( KEY_LOAD_K ),
			getenv( KEY_LOAD_R ),
			getenv( KEY_LOAD_DT), 
			DRAM_KERNEL_BOOTM,
...
static int do_ezb_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if ( 0 == strcmp( "ufk", argv[0] ) )
	{
		...
		// env, load kernel
		sprintf( str_env, "nand read 0x%x 0x%x 0x%x", DRAM_KERNEL_BOOTM, NAND_ADDR_KERNEL, ssize );	
	...
	else if ( 0 == strcmp( "ufka", argv[0] ) )   // add:lbb
	{
		...
		// env, load kernel
		sprintf( str_env, "nand read 0x%x 0x%x 0x%x", DRAM_KERNEL_BOOTM, NAND_ADDR_APP_KERNEL, ssize);
	...
	else if ( 0 == strcmp( "ufr", argv[0] ) )
	{
		...
		// env, load ramdisk
		sprintf( str_env, "nand read 0x%x 0x%x 0x%x", DRAM_RAMDISK_ADDR, NAND_ADDR_RAMDISK, ssize );	
	...
	else if ( 0 == strcmp( "ufd", argv[0] ) )
	{
		...
		// env, load dt
		sprintf( str_env, "nand read 0x%x 0x%x 0x%x", DRAM_DTREE_BOOTM, NAND_ADDR_DT, ssize );	
